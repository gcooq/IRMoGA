...
if online == False:
    ...
    def getXs():
        fpointvec = open('embeddings/GW_embedding_graph.dat', 'r')  # obtain the embeddings
        item = 0
...
def read_pois_r():
    ...
    Test_USER = []
    T_DATA = {}
    fread_train = open('data/gw/GW_Rtrain.dat', 'r')
    ...
    fread_train = open('data/gw/GW_Rtest.dat', 'r')
    for lines in fread_train.readlines():
        line = lines.split()
        data_line = list()
        for i in line[1:]:
...
def read_pois_f():
    ...
    Test_USER = []
    T_DATA = {}
    fread_train = open('data/gw/GW_finaltrain.txt', 'r')
    for lines in fread_train.readlines():
        line = lines.split()
        data_line = list()
        for i in line[1:]:
        ...
        Train_USER.append(line[0])
        T_DATA.setdefault(line[0], []).append(data_line)
    fread_train = open('data/gw/GW_finaltest.txt', 'r')
    for lines in fread_train.readlines():
        line = lines.split()
        data_line = list()
        for i in line[1:]:
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, pad_Au_out, pad_Au_in, training, mask):  # encoder
        ...
        # print(tf.shape(gnn))
        his_attn_output, _ = self.batt(out2, gnn)  # (batch_size, input_seq_len, d_model)#k,v,q
