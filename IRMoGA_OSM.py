...
if online == False:
    ...
    def getXs():  # 读取轨迹向量
        fpointvec = open('embeddings/Boston_deepwalk_2022_128.dat', 'r')  # 获取check-in向量 已经用word2vec训练得到 tor traindata/
        #     table_X={}  #建立字典索引
        item = 0
...
def read_pois_r():
    ...
    Test_USER = []
    T_DATA = {}
    fread_train = open('data/Boston2/Boston_grace_input_split_train1.txt', 'r')
    ...
    fread_train = open('data/Boston2/Boston_grace_input_split_test1.txt', 'r')
    for lines in fread_train.readlines():
        line = lines.split()
        data_line = list()
        for i in line[1:]:
...
def read_pois_f():
    ...
    Test_USER = []
    T_DATA = {}
    fread_train = open('data/Boston2/Boston_grace_output_split_train1.txt', 'r')#finaltrain.txt
    for lines in fread_train.readlines():
        line = lines.split()
        data_line = list()
        for i in line[1:]:
        ...
        Train_USER.append(line[0])
        T_DATA.setdefault(line[0], []).append(data_line)
    fread_train = open('data/Boston2/Boston_grace_output_split_test1.txt', 'r')
    for lines in fread_train.readlines():
        line = lines.split()
        data_line = list()
        for i in line[1:]:
